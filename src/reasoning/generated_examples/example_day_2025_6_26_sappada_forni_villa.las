% Example generated data for day (2025, 6, 26)

#pos(e43@1000,{ 

forecasted_sky(sappada_forni_villa, "partly_cloudy", summer),
forecasted_rain(sappada_forni_villa, 2, summer)},
{
sunny_at(sappada_forni_villa,summer), 
covered_at(sappada_forni_villa,summer), 
rains_at(sappada_forni_villa,0,summer), 
rains_at(sappada_forni_villa,1,summer), 
rains_at(sappada_forni_villa,4,summer), 
rains_at(sappada_forni_villa,6,summer)
},
{
location_considered(sappada_forni_villa). 
%to drive the season (winter, spring, summer, autumn)
date(2025, 6, 26).

% Cloud coverage data:
% Cloud_covers(location,cloud_id,hh)
cloud_at_750m_covers(gorizia,654, 6).
cloud_at_1_4km_covers(pordenone,893, 4).
cloud_at_1_4km_covers(gorizia,894, 11).
cloud_at_1_4km_covers(sappada_forni_villa,901, 19).
cloud_at_1_4km_covers(pontebba_tarvisio,903, 21).
cloud_at_1_4km_covers(pordenone,906, 23).
cloud_at_3km_covers(sappada_forni_villa,1013, 22).
cloud_at_5_5km_covers(sappada_forni_villa,1008, 16).
cloud_at_5_5km_covers(gemona_stolvizza,1008, 16).
cloud_at_5_5km_covers(sappada_forni_villa,1012, 19).
cloud_at_5_5km_covers(sappada_forni_villa,1016, 20).
cloud_at_5_5km_covers(barcis,1016, 20).
cloud_at_9km_covers(barcis,1183, 10).
cloud_at_9km_covers(gemona_stolvizza,1183, 11).
cloud_at_9km_covers(sappada_forni_villa,1194, 12).
cloud_at_9km_covers(barcis,1194, 12).
cloud_at_9km_covers(pordenone,1198, 13).
cloud_at_9km_covers(barcis,1212, 15).
cloud_at_9km_covers(sappada_forni_villa,1212, 16).
cloud_at_9km_covers(gemona_stolvizza,1212, 16).
cloud_at_9km_covers(barcis,1207, 16).
cloud_at_9km_covers(sappada_forni_villa,1217, 19).
cloud_at_9km_covers(pontebba_tarvisio,1217, 19).
cloud_at_9km_covers(gemona_stolvizza,1217, 19).
cloud_at_9km_covers(sappada_forni_villa,1217, 20).
cloud_at_9km_covers(barcis,1217, 20).
cloud_at_9km_covers(pordenone,1217, 20).
cloud_at_9km_covers(lignano_grado,1217, 21).
cloud_at_9km_covers(barcis,1217, 21).
cloud_at_9km_covers(udine_palmanova,1217, 21).
cloud_at_9km_covers(sappada_forni_villa,1217, 22).
cloud_at_9km_covers(pontebba_tarvisio,1217, 22).
cloud_at_9km_covers(lignano_grado,1217, 22).
cloud_at_9km_covers(barcis,1217, 22).
cloud_at_9km_covers(udine_palmanova,1217, 22).
cloud_at_9km_covers(gorizia,1217, 22).
cloud_at_9km_covers(gemona_stolvizza,1217, 22).
cloud_at_9km_covers(sappada_forni_villa,1217, 23).
cloud_at_9km_covers(pontebba_tarvisio,1217, 23).
cloud_at_9km_covers(lignano_grado,1217, 23).
cloud_at_9km_covers(barcis,1217, 23).
cloud_at_9km_covers(udine_palmanova,1217, 23).
cloud_at_9km_covers(trieste,1217, 23).
cloud_at_9km_covers(gemona_stolvizza,1217, 23).
cloud_at_9km_covers(pordenone,1217, 23).

%summing up temperature and humidity facts 

% temperature_at_morning(sappada_forni_villa,277.07).
% temperature_at_afternoon(sappada_forni_villa,277.56).
temperature_increased_at_afternoon(sappada_forni_villa).
% temperature_at_morning(pontebba_tarvisio,275.27).
% temperature_at_afternoon(pontebba_tarvisio,277.23).
temperature_increased_at_afternoon(pontebba_tarvisio).
% temperature_at_morning(lignano_grado,277.83).
% temperature_at_afternoon(lignano_grado,276.71).
temperature_decreased_at_afternoon(lignano_grado).
% temperature_at_morning(barcis,278.10).
% temperature_at_afternoon(barcis,277.54).
temperature_decreased_at_afternoon(barcis).
% temperature_at_morning(udine_palmanova,277.03).
% temperature_at_afternoon(udine_palmanova,275.79).
temperature_decreased_at_afternoon(udine_palmanova).
% temperature_at_morning(gorizia,277.80).
% temperature_at_afternoon(gorizia,276.12).
temperature_decreased_at_afternoon(gorizia).
% temperature_at_morning(trieste,278.13).
% temperature_at_afternoon(trieste,276.83).
temperature_decreased_at_afternoon(trieste).
% temperature_at_morning(gemona_stolvizza,278.37).
% temperature_at_afternoon(gemona_stolvizza,277.25).
temperature_decreased_at_afternoon(gemona_stolvizza).
% temperature_at_morning(pordenone,277.27).
% temperature_at_afternoon(pordenone,276.23).
temperature_decreased_at_afternoon(pordenone).
% humidity_at_morning(sappada_forni_villa,34.67).
% humidity_at_afternoon(sappada_forni_villa,42.50).
humidity_increased_at_afternoon(sappada_forni_villa).
% humidity_at_morning(pontebba_tarvisio,42.00).
% humidity_at_afternoon(pontebba_tarvisio,52.50).
humidity_increased_at_afternoon(pontebba_tarvisio).
% humidity_at_morning(lignano_grado,46.00).
% humidity_at_afternoon(lignano_grado,45.42).
humidity_decreased_at_afternoon(lignano_grado).
% humidity_at_morning(barcis,41.33).
% humidity_at_afternoon(barcis,44.58).
humidity_increased_at_afternoon(barcis).
% humidity_at_morning(udine_palmanova,60.00).
% humidity_at_afternoon(udine_palmanova,42.92).
humidity_decreased_at_afternoon(udine_palmanova).
% humidity_at_morning(gorizia,53.33).
% humidity_at_afternoon(gorizia,43.75).
humidity_decreased_at_afternoon(gorizia).
% humidity_at_morning(trieste,62.00).
% humidity_at_afternoon(trieste,47.50).
humidity_decreased_at_afternoon(trieste).
% humidity_at_morning(gemona_stolvizza,45.33).
% humidity_at_afternoon(gemona_stolvizza,40.00).
humidity_decreased_at_afternoon(gemona_stolvizza).
% humidity_at_morning(pordenone,48.67).
% humidity_at_afternoon(pordenone,61.25).
humidity_increased_at_afternoon(pordenone).
wind_blowing_morning(sappada_forni_villa,"NE",11).
wind_blowing_afternoon(sappada_forni_villa,"NE",11).
wind_blowing_morning(pontebba_tarvisio,"NE",10).
wind_blowing_afternoon(pontebba_tarvisio,"NE",11).
wind_blowing_morning(lignano_grado,"NE",10).
wind_blowing_afternoon(lignano_grado,"NE",12).
wind_blowing_morning(barcis,"NE",11).
wind_blowing_afternoon(barcis,"NE",11).
wind_blowing_morning(udine_palmanova,"NE",9).
wind_blowing_afternoon(udine_palmanova,"NE",11).
wind_blowing_morning(gorizia,"NE",9).
wind_blowing_afternoon(gorizia,"NE",11).
wind_blowing_morning(trieste,"NE",9).
wind_blowing_afternoon(trieste,"NE",11).
wind_blowing_morning(gemona_stolvizza,"NE",10).
wind_blowing_afternoon(gemona_stolvizza,"NE",11).
wind_blowing_morning(pordenone,"NE",10).
wind_blowing_afternoon(pordenone,"NE",12).

temp_front_morning_at_100m(lignano_grado,udine_palmanova).
temp_front_morning_at_100m(gemona_stolvizza,udine_palmanova).
temp_front_morning_at_100m(gorizia,lignano_grado).
temp_front_afternoon_at_100m(lignano_grado,udine_palmanova).
temp_front_afternoon_at_100m(gorizia,lignano_grado).
temp_front_afternoon_at_100m(gemona_stolvizza,udine_palmanova).
temp_front_morning_at_750m(gorizia,pontebba_tarvisio).
temp_front_morning_at_750m(gemona_stolvizza,pontebba_tarvisio).
temp_front_morning_at_750m(sappada_forni_villa,udine_palmanova).
temp_front_morning_at_750m(barcis,sappada_forni_villa).
temp_front_morning_at_750m(pordenone,sappada_forni_villa).
temp_front_afternoon_at_750m(gemona_stolvizza,udine_palmanova).
temp_front_morning_at_1_4km(pontebba_tarvisio,sappada_forni_villa).
temp_front_morning_at_1_4km(gorizia,pontebba_tarvisio).
temp_front_morning_at_1_4km(gemona_stolvizza,pontebba_tarvisio).
temp_front_afternoon_at_1_4km(pontebba_tarvisio,sappada_forni_villa).
temp_front_afternoon_at_1_4km(gemona_stolvizza,pontebba_tarvisio).
temp_front_morning_at_5_5km(sappada_forni_villa,udine_palmanova).
temp_front_morning_at_5_5km(barcis,pordenone).
temp_front_morning_at_5_5km(gorizia,pontebba_tarvisio).
temp_front_morning_at_5_5km(gemona_stolvizza,udine_palmanova).
temp_front_morning_at_5_5km(pordenone,sappada_forni_villa).
temp_front_afternoon_at_5_5km(gorizia,pontebba_tarvisio).

hum_front_morning_at_100m(sappada_forni_villa,udine_palmanova).
hum_front_morning_at_100m(lignano_grado,udine_palmanova).
hum_front_morning_at_100m(gemona_stolvizza,udine_palmanova).
hum_front_afternoon_at_100m(sappada_forni_villa,udine_palmanova).
hum_front_afternoon_at_100m(lignano_grado,trieste).
hum_front_afternoon_at_100m(gorizia,pontebba_tarvisio).
hum_front_afternoon_at_100m(gemona_stolvizza,udine_palmanova).
hum_front_afternoon_at_100m(pordenone,udine_palmanova).
hum_front_morning_at_3km(gorizia,pontebba_tarvisio).
hum_front_morning_at_3km(pontebba_tarvisio,sappada_forni_villa).
hum_front_morning_at_3km(gemona_stolvizza,pontebba_tarvisio).
hum_front_afternoon_at_3km(sappada_forni_villa,udine_palmanova).
hum_front_afternoon_at_3km(barcis,pordenone).
hum_front_afternoon_at_3km(pordenone,sappada_forni_villa).
hum_front_afternoon_at_3km(pontebba_tarvisio,sappada_forni_villa).
hum_front_afternoon_at_3km(gemona_stolvizza,sappada_forni_villa).
hum_front_morning_at_5_5km(sappada_forni_villa,udine_palmanova).
hum_front_morning_at_9km(sappada_forni_villa,udine_palmanova).
hum_front_morning_at_9km(gorizia,pontebba_tarvisio).
hum_front_morning_at_9km(barcis,pordenone).
hum_front_morning_at_9km(pordenone,sappada_forni_villa).

}). 
