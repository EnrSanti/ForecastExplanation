...
def extract_day(dates: List[datetime], region: Region, output_dir, clean_level: int = 0) -> None:
    logger.info("Starting data extraction...")

    with ProcessPoolExecutor(max_workers=12) as executor:
        futures = {
            executor.submit(extract_day_worker, date, region, output_dir, clean_level): date
            for date in dates
        }
...
def extract(dates: List[datetime], region: Region, output_dir: str, clean_level: int = 0) -> None:
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(RAW_DATA_DIR, exist_ok=True)
    os.makedirs(CUT_DATA_DIR, exist_ok=True)
    os.makedirs(DISCRETE_DATA_DIR, exist_ok=True)
    ...
    extract_day(dates, region, output_dir, clean_level)
