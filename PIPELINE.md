...
Reads the raw GRIB file for a given date, decodes it via cfgrib, cuts it to the configured region, and produces two outputs.
...
Reads the extracted `features.nc`, runs TOBAC blob detection and tracking on the normalized fields, and produces three separate output files.
...
Reads both `segmentation.nc` and `features.nc` from Step 2, cross-references TOBAC blobs with physical values, and produces tabular `.txt` reports.
...
#### `winds.txt`
Mean wind within a 3 km radius of each city, per height and hour. Direction is the magnitude-weighted vector mean, reported as a compass octave.
...
#### `cloud.txt`
Cloud segments detected by TOBAC that overlap each city. One row per cloud–city intersection, per height and hour. Coverage is the percentage of the city's radius covered by the cloud.
