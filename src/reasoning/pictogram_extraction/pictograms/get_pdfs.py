...
def fetch_previs_and_pdf(day, month, year):
    params = {"a": year, "m": month, "g": day, "z": "regione", "l": "it", "ln": ""}
    ...
    matches = re.findall(
        r"pdf/\d{4}/\d{8}/(\d{12})/pdf/regione-\d{8}-it\.pdf", clean_html
    )
    print(clean_html)
    if not matches:
    ...
    dd = f"{day:02d}"
    pdf_url = (
        f"{PDF_BASE}pdf/{year}/{yyyymmdd}/{timestamp}/pdf/regione-{yyyymmdd}-it.pdf"
    )
